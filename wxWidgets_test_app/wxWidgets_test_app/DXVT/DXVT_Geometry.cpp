...
int g_nQuadTreeIndex;

DXVTBaseGeometry::DXVTBaseGeometry(DXVTDevice *pDXVTDevice)
{
...
DXVTBaseGeometry::~DXVTBaseGeometry()
{

}

void DXVTBaseGeometry::CreateVertexBuffer(size_t Size,DWORD dwFormat,int nNumVert)
{
	 (*m_pDXVTDevice)->CreateVertexBuffer(
                   Size, 
                   D3DUSAGE_WRITEONLY,
                   dwFormat,
                   D3DPOOL_MANAGED,
                   &m_pVertexBuffer,
                   0);
   		 m_vertSize = Size/nNumVert;
		 m_nNumVert = nNumVert;
		 m_dwFormat = dwFormat;
}

void DXVTBaseGeometry::CreateIndexBuffer16(size_t Size)
{
	     (*m_pDXVTDevice)->CreateIndexBuffer(
                   Size,
                   D3DUSAGE_WRITEONLY,
                   D3DFMT_INDEX16,
                   D3DPOOL_MANAGED,
...
void DXVTBaseGeometry::CreateIndexBuffer32(size_t Size)
{
	     (*m_pDXVTDevice)->CreateIndexBuffer(
                   Size,
                   D3DUSAGE_WRITEONLY,
...
void DXVTBaseGeometry::SetIndex()
{
	m_pIndexBuffer->Unlock();
}

void DXVTBaseGeometry::SetVertex()
{
	m_pVertexBuffer->Unlock();
}

void DXVTBaseGeometry::GetIndex(void ** ppIndices)
{
m_pIndexBuffer->Lock(0, 0, ppIndices, 0);
}

void DXVTBaseGeometry::GetVertex(void ** ppVertices)
{
m_pVertexBuffer->Lock(0, 0, ppVertices, 0);
}

void DXVTBaseGeometry::Draw(int nNumPrimitives,String::VString vszPrimitiveType)
{
		  (*m_pDXVTDevice)->SetStreamSource(0, m_pVertexBuffer, 0, m_vertSize);
		  if(m_pIndexBuffer)
          (*m_pDXVTDevice)->SetIndices(m_pIndexBuffer);
          (*m_pDXVTDevice)->SetFVF(m_dwFormat);
		  if(vszPrimitiveType == "Triangle" && m_pIndexBuffer != NULL)
		  {
          (*m_pDXVTDevice)->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0, 0, m_nNumVert, 0, nNumPrimitives);
		  }
		  else 
		  {
		  (*m_pDXVTDevice)->DrawPrimitive(D3DPT_TRIANGLELIST,0,nNumPrimitives);
		  }
		  if(vszPrimitiveType == "Line" && m_pIndexBuffer)
		  (*m_pDXVTDevice)->DrawIndexedPrimitive(D3DPT_LINELIST,0, 0, m_nNumVert, 0, nNumPrimitives);
		  else
		  (*m_pDXVTDevice)->DrawPrimitive(D3DPT_LINELIST,0,nNumPrimitives);
		  if(vszPrimitiveType == "Point" && m_pIndexBuffer)
		  (*m_pDXVTDevice)->DrawIndexedPrimitive(D3DPT_POINTLIST,0, 0, m_nNumVert, 0, nNumPrimitives);
		  else
		  (*m_pDXVTDevice)->DrawPrimitive(D3DPT_POINTLIST,0,nNumPrimitives);
}
...
DXVTRay::DXVTRay(D3DXVECTOR3 Origin,D3DXVECTOR3 Direction)
{
	m_Origin = Origin;
