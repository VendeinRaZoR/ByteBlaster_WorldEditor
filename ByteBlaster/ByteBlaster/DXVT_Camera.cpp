...
DXVTCamera::DXVTCamera(DXVTDevice *pVTGLDevice)
{
...
void DXVTCamera::SetProjectionMatrix(float fFOV,float fDistance)
{
	...
	m_mProjection = *VTGL_Perspective(fFOV,((float)m_pVTGLDevice->GetResolution().x / (float)m_pVTGLDevice->GetResolution().y),1.0f,fDistance);
